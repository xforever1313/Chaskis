...
{
    [TestFixture]
    public sealed class SendMessageEventArgsTests
    {
        // ---------------- Fields ----------------
        ...
        {
            SendMessageEventArgs uut = new SendMessageEventArgs
            {
                Protocol = protocol,
                Server = server,
        ...
        public void InvalidXmlRootName()
        {
            ...
            Assert.Throws<ValidationException>(
                () => SendMessageEventArgsExtensions.FromXml( xmlString, mockWriter.Object )
            );
        }
        ...
        {
            string xmlString = $"<chaskis_sendmessage_event><protocol>{protocol}</protocol><channel>{channel}</channel><message>{message}</message></chaskis_sendmessage_event>";
        ...
        {
            string xmlString = $"<chaskis_sendmessage_event><server>{server}</server><channel>{channel}</channel><message>{message}</message></chaskis_sendmessage_event>";
        ...
        {
            string xmlString = $"<chaskis_sendmessage_event><server>{server}</server><protocol>{protocol}</protocol><message>{message}</message></chaskis_sendmessage_event>";
        ...
        {
            string xmlString = $"<chaskis_sendmessage_event><server>{server}</server><protocol>{protocol}</protocol><channel>{channel}</channel></chaskis_sendmessage_event>";
        ...
        private void DoRoundTripTest( SendMessageEventArgs uut )
        {
            string xmlString = uut.ToXml();
            SendMessageEventArgs postXml = SendMessageEventArgsExtensions.FromXml( xmlString, mockWriter.Object );
