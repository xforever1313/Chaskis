...
{
    [TestFixture]
    public sealed class SendNoticeEventArgsTests
    {
        // ---------------- Fields ----------------
        ...
        {
            SendNoticeEventArgs uut = new SendNoticeEventArgs
            {
                Protocol = protocol,
                Server = server,
        ...
        public void InvalidXmlRootName()
        {
            ...
            Assert.Throws<ValidationException>(
                () => SendNoticeEventArgsExtensions.FromXml( xmlString, mockWriter.Object )
            );
        }
        ...
        {
            string xmlString = $"<chaskis_sendnotice_event><protocol>{protocol}</protocol><channel>{channel}</channel><message>{message}</message></chaskis_sendnotice_event>";
        ...
        {
            string xmlString = $"<chaskis_sendnotice_event><server>{server}</server><channel>{channel}</channel><message>{message}</message></chaskis_sendnotice_event>";
        ...
        {
            string xmlString = $"<chaskis_sendnotice_event><server>{server}</server><protocol>{protocol}</protocol><message>{message}</message></chaskis_sendnotice_event>";
        ...
        {
            string xmlString = $"<chaskis_sendnotice_event><server>{server}</server><protocol>{protocol}</protocol><channel>{channel}</channel></chaskis_sendnotice_event>";
        ...
        private void DoRoundTripTest( SendNoticeEventArgs uut )
        {
            string xmlString = uut.ToXml();
            SendNoticeEventArgs postXml = SendNoticeEventArgsExtensions.FromXml( xmlString, mockWriter.Object );
